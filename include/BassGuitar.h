#ifndef STK_BassGuitar_H
#define STK_BassGuitar_H
...
class BassGuitar : public Stk
{
 public:
  //! Class constructor, specifying an arbitrary number of strings (default = 4).
  BassGuitar( unsigned int nStrings = 4, std::string bodyfile = "" );
...
inline StkFloat BassGuitar :: tick( StkFloat input )
{
...
inline StkFrames& BassGuitar :: tick( StkFrames& frames, unsigned int channel )
{
#if defined(_STK_DEBUG_)
  if ( channel >= frames.channels() ) {
    oStream_ << "BassGuitar::tick(): channel and StkFrames arguments are incompatible!";
...
inline StkFrames& BassGuitar :: tick( StkFrames& iFrames, StkFrames& oFrames, unsigned int iChannel, unsigned int oChannel )
{
#if defined(_STK_DEBUG_)
